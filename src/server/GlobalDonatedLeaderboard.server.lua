...
local ODS = DataStoreService:GetOrderedDataStore("TipJar_Donated_ORD_v1")
local robuxSymbol = utf8.char(0xE002)

local donatedBoardModel = workspace:WaitForChild("DonatedLeaderboard")
local ScoreBlock        = donatedBoardModel:WaitForChild("ScoreBlock")
...
local function extractUserIdFromKey(key: string): number?
	...
	local idStr = key:match("donated_(%d+)")
	if idStr then
...
local function updateLeaderboard()
	...
	if not success then
		warn("[GlobalDonatedLeaderboard] GetSortedAsync failed:", pages)
		return
	end
	...
	for _, entry in ipairs(entries) do
		...
		if userId then
			local donated = tonumber(entry.value) or 0
			if donated < 0 then donated = 0 end
			table.insert(topUsers, { userId = userId, donated = donated })
	...
	for rank, info in ipairs(topUsers) do
		...
		if valueLabel and valueLabel:IsA("TextLabel") then
			valueLabel.Text = tostring(donated) .. robuxSymbol
		end
