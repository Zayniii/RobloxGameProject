-- ServerScriptService/GlobalRaisedLeaderboard.server.lua

local Players          = game:GetService("Players")
local DataStoreService = game:GetService("DataStoreService")

-- OrderedDataStore für RAISED (Robux erhalten)
local ODS = DataStoreService:GetOrderedDataStore("TipJar_Raised_ORD_v1")

-- Robux-Symbol
local robuxSymbol = utf8.char(0xE002)

-- == GUI-Referenzen ==
local raisedBoardModel = workspace:WaitForChild("RaisedLeaderboard")
local ScoreBlock       = raisedBoardModel:WaitForChild("ScoreBlock")
...
local function extractUserIdFromKey(key: string): number?
	...
	local idStr = key:match("raised_(%d+)")
	if idStr then
...
local function updateLeaderboard()
	-- ODS lesen: Top N nach Raised
	local success, pages = pcall(function()
		-- false = absteigend (größter Wert zuerst)
		return ODS:GetSortedAsync(false, 80)  -- etwas mehr holen, falls was rausfliegt
	end)

	if not success then
		warn("[GlobalRaisedLeaderboard] GetSortedAsync failed:", pages)
		return
	end

	local entries = pages:GetCurrentPage()
	local topUsers = {}  -- { {userId=, raised=} ... }

	for _, entry in ipairs(entries) do
		local userId = extractUserIdFromKey(entry.key)
		if userId then
			local raised = tonumber(entry.value) or 0
			if raised < 0 then raised = 0 end
	...
	for rank, info in ipairs(topUsers) do
		...
		local valueLabel     = row:FindFirstChild("PlaytimeLabel", true) -- wird hier für Raised genutzt
		...
		if valueLabel and valueLabel:IsA("TextLabel") then
			valueLabel.Text = tostring(raised) .. robuxSymbol
		end
	...
	-- Zeilen entfernen, die nicht mehr im Top-Board sind
